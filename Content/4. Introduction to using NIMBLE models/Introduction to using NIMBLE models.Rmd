...
Get names of nodes on the graph
=====
```{r}
model$getNodeNames()
```
...
mc2 <- nimbleCode({
    ...
    slope ~ dnorm(0, sd = 1000)
    sigma ~ dunif(0, 100)
    predicted.y[1:4] <- intercept + slope * x[1:4]
    for(i in 1:4) {
        y[i] ~ dnorm(predicted.y[i], sd = sigma)
    }
})
...
These are called *lifted nodes*.  There were created by lifting an expression out of one distribution and creating a deterministic node for that expression.
