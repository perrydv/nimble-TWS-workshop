...
- For estimation, assume we know residual standard deviation so can look at two-dimensional posteriors (slope, intercept).
...
- Denominator is hard to calculate but it is a constants.
- Since there are no random effects, we can calculate the posterior up to a constant easily.
...
intercept_grid <- lmCoef['(Intercept)', 'Estimate'] + lmCoef['(Intercept)', 'Std. Error'] * seq(-3, 3, length = 21)
slope_grid <- lmCoef['lmModel$x', 'Estimate'] + lmCoef['lmModel$x', 'Std. Error'] * seq(-3, 3, length = 21)
...
=====
With many dimensions of parameters (including random effects or latent states), we want to approximate the density with a sample from it.

```{r}
## we will "cheat" at use the mle
library(mvtnorm)
...
One-dimensional MCMC: Conditional likelihood surface
=====
...
- Usually only the part of $[\mbox{data | parameters}]$ that involves a particular parameters (e.g. intercept) needs to be calculated.
...
- Mixing is how quickly they move around the distribution.
...
- We can write [intercept | slope, data] analytically.
...
- Again only parts of $[\mbox{data, parameters}]$ involving $\theta$ are required.
- Hard to generalize to multivariate (block) sampling.
