...
Code is modified from BPA book:
```{r}
...
Function for jags-like sampler configuration
=====
```{r}
...
Function for no-conjugacy sampler configuration
=====
```{r}
...
Function for sampler configuration handling sigmas on log scale
=====
```{r}
...
Function for sampler configuration with two samplers for each sigma.
=====
```{r}
...
make_MCMC_comparison_pages(martin,
                           dir = "martin_results_with_slides",
                           modelName = "martin-state-space-model")
```

```{r include=FALSE}
for(v in names(martin)) {
  martin[[v]]$samples <- NULL
}
saveRDS(martin, file = file.path("martin_results_with_slides", "martin.RDS"))
```

```{r eval=FALSE}
make_MCMC_comparison_pages(martin,
                           dir = "martin_results_user",
                           modelName = "martin-state-space-model")
```

Results are [here](martin_results_with_slides/martin-state-space-model.html)

```{r include=FALSE}
martin <- readRDS(file.path("martin_results_with_slides", "martin.RDS"))
```

```{r}
combineMetrics(martin)
```