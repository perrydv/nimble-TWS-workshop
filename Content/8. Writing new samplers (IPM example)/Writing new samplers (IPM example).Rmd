...
samples <- runMCMC(Cipm_mcmc, niter = ni, nburnin = nb)
colnames(samples)
```
...
plot(samples[,'N[1, 1]'], pch = '.') ## yuck
plot(samples[,'N[1, 2]'], pch = '.') ## much better
plot(samples[,'mean.p'], pch = '.') ## looks pretty good
## look at some pairs, dropped burnin = 500
pairs(samples[, 1:8], pch = '.')
...
pairs(samples[, 9:16], pch = '.')
## skip some
pairs(samples[, 21:28], pch = '.') 
## Look at ESS:
library(coda)
...
=====
```{r}
combineMetrics(ipm_comparison)
```

Make figures of results
=====
```{r}
catcher <- try(make_MCMC_comparison_pages(ipm_comparison,
                                          dir = "ipm_comparison",
...
In this case, we see that the posteriors are different.

This requires investigation.

- In theory, all the MCMCs should be sampling the same posterior.
- It may be "jags" and "nimble" have a complicated case of poor mixing.  Initial state values still mix poorly.
- It may be that providing more complete initial values would help.
- It may be that the "nimbleCustom" has a problem, but its results look most reasonable.
