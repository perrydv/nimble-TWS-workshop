...
- There is essentially no posterior probability of including any covariates.
- `beta[i]`s follow their priors, except for rare moments of inclusion in the model.
- JAGS uses slice samplers.
- Once a `z[i]` is 0, the corresponding `beta[i]` will follow its prior with quite good mixing and only rarely take a value that allows `z[i]` to go to 1.
- A common work-around is to use informative priors.  That means changing your model to help the MCMC implementation.
...
- The mixing is poor, but it seems to give a more reasonable result than JAGS.
...
- This creates poor mixing over `z[i]`s.
     ...
     - With a random-walk MH sampler (default nimble), adaptation depends on `z[i]` (is it adapting to prior or posterior?), and this affects mixing when `z[i]` is 0.  Behavior may be variable, but it gives more inclusions (`z[i]` = 1).
     
- JAGS use of slice samplers seems to lead to bad results in this example.

- `nimble` use of RW MH samplers seems to give more sensible results but still has issues.
...
- From this one example, JAGS (slice samplers, really) may not have reliable behavior with these problems, and default nimble (RW MH samplers, really) may have more reliable but still problematic behavior.
